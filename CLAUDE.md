...
`spazioricci/catalogo-carte`). Si aggiorna da solo ogni notte. Interfaccia in **italiano**.
...
scripts/
  ...
  build.mjs         # templates + cards.json -> dist/
...
templates/          # HTML template (home, catalogo, scheda mazzo)
...
- I template in `templates/` usano token `{{campo}}` e blocchi commentati `<!-- ITEM -->…<!-- /ITEM -->`
  che `build.mjs` ripete per ogni card. Vedi commenti dentro i template per l'elenco dei token.
- Pagine generate: `dist/index.html`, `dist/catalogo/index.html`, `dist/mazzo/<id>.html`.
- Il filtraggio/ricerca del catalogo è **lato client** leggendo `dist/data/cards.json`.
